...
//removing all samplers so can play purely on the web untill i figure the audiocontext stuff
...
function setup(){
  ...
  modeButton.position(330-80, 80+120);
  ...
  melodySlider = createSlider(-24, 4, -12, 1);
  synthSlider = createSlider(-24, -6, -20, 1);
}
...
function draw(){
  ...
  synth1.volume.value = synthSlider.value();
  synth2.volume.value = synthSlider.value();
  synth3.volume.value = melodySlider.value();
  synth4.volume.value = bassSlider.value();
  complexityValue = complexitySlider.value();

//stopping layer if volume == min
  if(synthScale.length>0 && Tone.Transport.state == "started" && synthState==true && synthSlider.value() > -23){
    synth.start("2n");
  ...
  if(synthScale.length>0 && Tone.Transport.state == "started" && melodyState==true && melodySlider.value() > -23){
      melody.start("2n");
  ...
  if(Tone.Transport.state == "started" && bassState==true && bassSlider.value() > -23){
    bass.start("4n");
